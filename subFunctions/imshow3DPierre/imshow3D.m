function  imshow3D( Img, disprange,varargin )
...
if ~isempty(opts.plot)
    plots = opts.plot;
    hold on
    for indplot =1:size(plots,3)
        plot(plots(S,:,indplot),'LineWidth',opts.LineWidth)
    end
    hold off
...
% -=< Mouse scroll wheel callback function >=-
    function mouseScroll (object, eventdata)
        ...
        obje = gca;
        obje = obje.Children;
        set(obje(end),'cdata',squeeze(Img(:,:,S,:)))
        for i =1:length(obje)-1
        set(obje(i),'Ydata',plots(S,:,i))
        end
    end
...
ind = find(strcmpi(UserInputs,'plot'));
if ~isempty(ind)
    opts.plot = UserInputs{ind + 1};
end
ind = find(strcmpi(UserInputs,'LineWidth'));
if ~isempty(ind)
    opts.LineWidth = UserInputs{ind + 1};
end
